...
Usage:
  ./hunyuan3d_batch.py <input_dir> <output_dir> [options]
...
def save_stl(vertices, faces, filepath):
    """Write binary STL from (N,3) and (M,3) torch tensors."""
    verts = vertices.cpu().numpy().astype(np.float32)
    tris  = faces.cpu().numpy().astype(np.int64)
...
def process_image(img_path, out_path, model, clip_vision, vae, args):
    ...
    # Step 1: CLIP vision encode (crop=center, matching workflow)
    print("  CLIP encode...")
    ...
    # Step 4: KSampler (euler / normal, matching workflow)
    ...
    voxel_obj = Types.VOXEL(raw_voxels)   # .data is a (B, D, H, W) tensor

    # Step 6: Voxel → mesh via surface-net algorithm
    print("  Mesh extraction (surface net)...")
    all_verts, all_faces = [], []
    offset = 0
    for vox_slice in voxel_obj.data:          # each slice is (D, H, W)
        v, f = voxel_to_mesh_surfnet(vox_slice, threshold=args.threshold)
        all_verts.append(v)
        all_faces.append(f + offset)
        offset += v.shape[0]

    vertices = torch.cat(all_verts)
    faces    = torch.cat(all_faces)

    # Step 7: Write binary STL
    print(f"  Saving → {out_path.name}")
    save_stl(vertices, faces, out_path)
    print(f"  OK  {vertices.shape[0]:,} verts  {faces.shape[0]:,} faces")
...
def main():
    ap = argparse.ArgumentParser(
        description="Batch image → STL via Hunyuan3D v2.1 (headless)",
    ...
    model, _clip, vae, clip_vision = comfy.sd.load_checkpoint_guess_config(
        ckpt_path,
        output_vae=True,
        output_clip=False,       # no text CLIP needed for Hunyuan3D
        output_clipvision=True,
    )

    # Apply AuraFlow shift patch (shift=1.0, multiplier=1.0 — matches workflow)
